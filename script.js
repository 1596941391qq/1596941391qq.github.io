    ...
    function toggleLang(){const h=document.documentElement,n=h.getAttribute('data-lang')==='zh'?'en':'zh';h.setAttribute('data-lang',n);h.lang=n==='zh'?'zh-CN':'en';document.getElementById('langToggle').textContent=n==='zh'?'EN':'中文';localStorage.setItem('lang',n);applyLang(n)}
    ...
    (function(){const t=localStorage.getItem('theme');if(t){document.documentElement.setAttribute('data-theme',t);document.getElementById('themeToggle').textContent=t==='dark'?'☀':'☽'}const l=localStorage.getItem('lang');if(l){document.documentElement.setAttribute('data-lang',l);document.documentElement.lang=l==='zh'?'zh-CN':'en';const _lt=document.getElementById('langToggle');if(_lt)_lt.textContent=l==='zh'?'EN':'中文';applyLang(l)}document.querySelectorAll('.cpyear').forEach(e=>e.textContent=new Date().getFullYear())})();
